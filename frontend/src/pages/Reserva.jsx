...
export default function Reserva() {
  ...
  useEffect(() => {
    if (!slug) return
    api(`${base}/negocio`).then((d) => setNegocio(d.nombre)).catch(() => setNoExiste(true))
  }, [slug, base])
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-slate-100 px-4 py-8">
      <div className="max-w-md mx-auto">
        ...
        <p className="text-slate-400 text-center text-sm mb-6">{negocio || 'Logix'}</p>
        ...
        {noExiste ? (
          ...
          <>
            ...
            {tab === 'reservar' ? <FormReserva base={base} /> : <MisCitas base={base} />}
          </>
        )}
      </div>
    </div>
...
function FormReserva({ base }) {
  const [servicios, setServicios] = useState([])
  const [form, setForm] = useState({ nombre_completo: '', email: '', telefono: '', servicio_id: '' })
  ...
  async function buscar() {
    ...
    const q = `${base}/disponibilidad?fecha=${fecha}${form.servicio_id ? `&servicio_id=${form.servicio_id}` : ''}`
  ...
  async function reservar(e) {
    ...
    try {
      const r = await api(`${base}/reservar`, { method: 'POST', body: { ...form, servicio_id: form.servicio_id || null, inicio: seleccion } })
  ...
  if (confirmada) {
    return (
      <div className="rounded-2xl bg-emerald-600/20 border border-emerald-500/40 p-6 text-center">
        ...
        <button onClick={() => { setConfirmada(null); setSlots([]); setForm({ nombre_completo: '', email: '', telefono: '', servicio_id: '' }) }}
        ...
        {citas?.map((c) => (
          <div key={c.id} className="rounded-lg border border-slate-800 bg-slate-800/50 p-3 flex justify-between items-center">
            <div>
              ...
              <p className="text-slate-400 text-sm">{c.servicio?.nombre ?? 'Cita'} · {c.estado}</p>
            </div>
